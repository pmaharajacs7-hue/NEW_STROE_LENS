...
import com.store.dto.SaleChartDTO;
...
public class SalesController {
    ...
    @GetMapping("/chart")
    public SaleChartDTO getChart(HttpServletRequest request) {
        ...
        List<SaleChartDTO.ProductSeries> series = new ArrayList<>();
        ...
        for (Map.Entry<Long, List<Sale>> entry : byProduct.entrySet()) {
            ...
            for (Sale s : productSales) {
                qtyByDate.merge(s.getDate(), s.getQuantity(), Integer::sum);
            }

            // Fill 10-day array
            int[] quantities = new int[10];
            for (int i = 0; i < 10; i++) {
                quantities[i] = qtyByDate.getOrDefault(days[i], 0);
            }

            series.add(new SaleChartDTO.ProductSeries(name, quantities));
        }

        return new SaleChartDTO(dates, series);
    }
}
